...
from net.helpers import (cosine_beta_schedule, linear_beta_schedule,
                     vp_beta_schedule, extract, Losses)
from net.utils import Progress, Silent
...
class Diffusion(nn.Module):
    ...
    def p_sample(self, x, t, s):  #bs,actiondim bs bs,statedim
    ...
    def p_sample_loop(self,
                      ...
                      shape,
                      verbose=False,
                      return_diffusion=False): #shape：(batch_size, self.action_dim)
...
if __name__ == '__main__':
    ...
    actor = Diffusion(state_dim=state_shape, action_dim=action_shape, model=actor_net,
                      max_action=10, beta_schedule='linear', n_timesteps=10).cuda()
    actor_optim = torch.optim.Adam(actor.parameters(),
                                   lr=0.01,
                                   weight_decay=0.01)
    # 假设你已经有了训练数据
    states = torch.randn(100, state_shape).cuda()  # 100 个状态样本
    actions = torch.randn(100, action_shape).cuda()  # 100 个动作样本
    ...
    # 生成动作
    state = torch.randn(10,state_shape).cuda()  # 随机生成一个状态
    action = actor.sample(state)  # 生成动作
    print(action.shape)
